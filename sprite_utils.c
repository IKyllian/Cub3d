...
/*   Updated: 2021/03/15 13:58:33 by kdelport         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
{
	ptr->sprite[i]->r_x = ((ptr->sprite[i]->x - ptr->player.pos_x)
			* ptr->fov.vect_x + (ptr->sprite[i]->y - ptr->player.pos_y)
			* ptr->fov.vect_y) / (sqrtf(powf(ptr->fov.vect_x, 2) + \
			powf(ptr->fov.vect_y, 2))
	...
	ptr->sprite[i]->r_x *= anglizer(ptr->fov.vect_x, ptr->fov.vect_y,
			ptr->sprite[i]->x - ptr->player.pos_x, ptr->sprite[i]->y - \
			ptr->player.pos_y);
}
